...
function dhf_get_article_prompt_context( $post_id, $content ) {
	...
	// Bolt optimization: Use get_the_terms() + wp_list_pluck() instead of wp_get_post_terms() to leverage object cache and prevent N+1 database query bottlenecks.
	$raw_categories = get_the_terms( $post_id, 'category' );
	$categories     = ! empty( $raw_categories ) && ! is_wp_error( $raw_categories ) ? wp_list_pluck( $raw_categories, 'name' ) : array();

	$raw_tags = get_the_terms( $post_id, 'post_tag' );
	$tags     = ! empty( $raw_tags ) && ! is_wp_error( $raw_tags ) ? wp_list_pluck( $raw_tags, 'name' ) : array();
