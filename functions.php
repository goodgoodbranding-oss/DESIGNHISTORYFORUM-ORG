...
function dhf_get_article_prompt_context( $post_id, $content ) {
	...
	// Bolt optimization: leverage object cache instead of direct DB query
	$cat_terms  = get_the_terms( $post_id, 'category' );
	$categories = is_array( $cat_terms ) ? wp_list_pluck( $cat_terms, 'name' ) : array();

	// Bolt optimization: leverage object cache instead of direct DB query
	$tag_terms  = get_the_terms( $post_id, 'post_tag' );
	$tags       = is_array( $tag_terms ) ? wp_list_pluck( $tag_terms, 'name' ) : array();
