...
function dhf_get_article_prompt_context( $post_id, $content ) {
	...
	// Bolt optimization: Use get_the_terms and wp_list_pluck to leverage object cache and avoid N+1 queries.
	$cat_objs   = get_the_terms( $post_id, 'category' );
	$categories = ( $cat_objs && ! is_wp_error( $cat_objs ) ) ? wp_list_pluck( $cat_objs, 'name' ) : array();
	$tag_objs   = get_the_terms( $post_id, 'post_tag' );
	$tags       = ( $tag_objs && ! is_wp_error( $tag_objs ) ) ? wp_list_pluck( $tag_objs, 'name' ) : array();
