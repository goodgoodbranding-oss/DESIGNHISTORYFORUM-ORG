...
function dhf_get_article_prompt_context( $post_id, $content ) {
	...
	// Bolt optimization: use get_the_terms and wp_list_pluck to leverage object cache and prevent N+1 queries.
	$cat_terms  = get_the_terms( $post_id, 'category' );
	$categories = ! is_wp_error( $cat_terms ) && ! empty( $cat_terms ) ? wp_list_pluck( $cat_terms, 'name' ) : array();
	$tag_terms  = get_the_terms( $post_id, 'post_tag' );
	$tags       = ! is_wp_error( $tag_terms ) && ! empty( $tag_terms ) ? wp_list_pluck( $tag_terms, 'name' ) : array();
