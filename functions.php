...
function dhf_get_article_prompt_context( $post_id, $content ) {
	...
	// Performance: get_the_terms() uses the object cache, whereas wp_get_post_terms()
	// forces a database query. Using wp_list_pluck to get names from the cached term objects.
	$category_terms = get_the_terms( $post_id, 'category' );
	$categories     = ! empty( $category_terms ) && ! is_wp_error( $category_terms ) ? wp_list_pluck( $category_terms, 'name' ) : array();
