...
function dhf_normalize_prompt_text( $text ) {
	// Bolt optimization: strip shortcodes before tag stripping to prevent data malformation
...
function dhf_get_article_prompt_context( $post_id, $content ) {
	...
	$categories_objs = get_the_terms( $post_id, 'category' );
	$categories      = $categories_objs && ! is_wp_error( $categories_objs ) ? wp_list_pluck( $categories_objs, 'name' ) : array();
	$tags_objs       = get_the_terms( $post_id, 'post_tag' );
	$tags            = $tags_objs && ! is_wp_error( $tags_objs ) ? wp_list_pluck( $tags_objs, 'name' ) : array();
